...
Desktop installs the exact official DSH 0.1.5-rc.1 source revision and resolves @sparkelf/dsh-plus@0.1.0-rc.25 from npm during profile materialization. Git, Node.js, npm, and pnpm remain system toolchain requirements on native Windows targets. The Desktop package keeps only its Supervisor runtime and Electron assets; it does not carry ordinary Plus tarballs, an fs-ext prebuild, a pnpm bundle, or source closure resources.
